...
### 1. 内存屏障与指令重排序
编译器和CPU为了优化性能会重新排序指令，这可能导致锁释放后数据还没写完的竞态问题。内存屏障确保release操作前的所有更新都已完成，防止编译器或CPU乱序，保证数据更新和锁释放的严格先后顺序。

### 2. Spinlock（自旋锁）特点
- 使用CPU原子指令实现，通过不断检测并设置寄存器值来获取锁
- 等待时会一直占用CPU，不断轮询
- 持有锁时必须关闭中断，避免中断处理导致的竞态
- 适合短时间持有的锁
- 不可重入

### 3. Sleep-lock（睡眠锁）特点
- 适合长时间持有的锁（如文件系统读写、磁盘I/O等）
- 等待锁时会让出CPU，避免忙等浪费CPU时间
- 不需要关闭中断，允许在持有锁时响应中断
- 类似信号量机制，线程睡眠等待唤醒信号
- 不可重入

### 4. Sleep-lock使用限制
- 不能在中断处理程序中使用（中断处理不能睡眠）
- 不能在自旋锁保护的临界区内使用（避免死锁）
- 可以在sleep-lock的临界区内嵌套使用自旋锁
...
- 操作系统通过中断唤醒等待的线程
- 线程重新获取锁并继续执行
- 整个过程保证互斥性，同一时间只有一个线程能访问特定资源

### 6. 两种锁的对比
- Spinlock：关中断、忙等、短时间操作
- Sleep-lock：不关中断、睡眠等待、长时间操作
